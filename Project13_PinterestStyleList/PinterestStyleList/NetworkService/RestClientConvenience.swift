...
extension RestClient {
  ...
  func fetchImageList(urlString: String, parameters: [String : AnyObject]?=nil,
                      size: Int?=nil,
                      completionHandler: @escaping CompletionHandler) -> URLSessionDataTask? {
    let task = taskForGetImageWith(urlString: urlString, parameters: nil,
                                   size: size, completionHandler: { result, error in
      if let error = error {
