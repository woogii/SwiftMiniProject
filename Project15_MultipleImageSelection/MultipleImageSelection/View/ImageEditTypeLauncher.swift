...
protocol ImageEditTypeLauncherDelegate : class {
...
class ImageEditTypeLauncher: NSObject {
  ...
  weak var delegate: ImageEditTypeLauncherDelegate?
  var imageIndex: Int!
  // MARK : - Initialization
  override init() {
    super.init()
  ...
  func showEditType(imageIndex: Int) {
    ...
    if let window: UIWindow = UIApplication.shared.keyWindow {
      ...
      UIView.animate(withDuration: 0.3, delay: 0, usingSpringWithDamping: 1,
        ...
        self.collectionView.frame = CGRect(x: 0, y: yCoordinate,
...
// MARK : - ImageEditTypeLauncher : UICollectionViewDataSource, UICollectionViewDelegate
extension ImageEditTypeLauncher : UICollectionViewDataSource, UICollectionViewDelegate {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellID,
                                                        for: indexPath) as? ImageEditTypeCell else {
      return ImageEditTypeCell()
    }
...
// MARK : - ImageEditTypeLauncher : UICollectionViewDelegateFlowLayout
extension ImageEditTypeLauncher : UICollectionViewDelegateFlowLayout {
  // MARK : - UICollectonViewDelegateFlowLayout Methods
