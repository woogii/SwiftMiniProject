...
struct Movie {
  ...
  static func requestMovieList(searchKeyword: String?, completionHandler: @escaping QueryResult) {
    ...
    URLSession.shared.dataTask(with: url, completionHandler: { (data, _, error) in
      ...
      let json = try? JSONSerialization.jsonObject(with: data, options: []) as? [String:Any],
        let dictArray = json?[Constants.TMDBResponseKeys.Results] as? [[String:Any]] {
  ...
  static func createURLBasedOnRequestType(searchKeyword: String?) -> URL? {
    ...
    let apiQuery = URLQueryItem(name: Constants.TMDBParameterKeys.ApiKey, value: Secret.ApiKey)
