...
class SlideInMenu: NSObject {
  ...
  weak var postListVC: PostListViewController?
  ...
  func registerNibAndInstantiateCollectionViewCell() {
    ...
    guard let cell = nib.instantiate(withOwner: nil, options: nil)[0] as? SlideMenuCollectionViewCell else {
      return
    }
    menuCell = cell
  }
  ...
  func slideInMenu(to window: UIWindow) {
    ...
    self.collectionView.frame = CGRect(x: 0,
                                       y: window.frame.height - self.collectionViewFrame.height,
                                       width: self.collectionViewFrame.width,
                                       height: self.collectionViewFrame.height)
  }
...
extension SlideInMenu : UICollectionViewDelegate, UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    ...
    guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId,
                                                        for: indexPath) as? SlideMenuCollectionViewCell else {
