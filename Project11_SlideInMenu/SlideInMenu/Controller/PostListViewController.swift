...
class PostListViewController: UIViewController {
  ...
  func loadSampleDataFromBundle() {
    ...
    if let pathURL = Bundle.main.url(forResource: Constants.Common.SampleJSONInput,
                                    withExtension: Constants.Common.JSONType) {
...
extension PostListViewController : UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    guard let cell = tableView.dequeueReusableCell(withIdentifier:
                    Constants.CellIdentifier.PostInfoTableViewCell,
                          for: indexPath) as? PostInformationTableViewCell else {
