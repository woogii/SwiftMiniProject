...
class ViewController: UIViewController {
  ...
  fileprivate func configureSearchTagCollectionView() {
    let cellNib = UINib(nibName: Constants.NibFiles.SearchKeywordCell, bundle: nil)
    guard let cell = (cellNib.instantiate(withOwner: nil, options: nil)[0] as?  SearchKeywordCollectionViewCell) else {
      return
    }
    sizingCell = cell
    searchView.keywordCollectionView.register(cellNib, forCellWithReuseIdentifier: Constants.CellIDs.SearchKeyword)
  ...
  fileprivate func configureNavigationBar() {
    ...
    navigationController?.navigationBar.barTintColor = Constants.Colors.NavigationBar
  }
  fileprivate func addTapGestureToSearchView() {
    let tapGestureRecognizer = UITapGestureRecognizer(target:self,
                                                      action: #selector(ViewController.serchViewTapped(_:)))
  ...
  func serchViewTapped(_ view: UIView) {
    ...
    guard let destinationVC = storyboard
                                .instantiateViewController(withIdentifier:
                                  Constants.StoryboardIDs.KeywordInputVCStoryboardID)
                                  as? KeywordInputViewController else {
      return
    }
...
extension ViewController : UICollectionViewDelegate, UICollectionViewDataSource {
  ...
  public func collectionView(_ collectionView: UICollectionView,
    ...
    guard let cell = collectionView
                      .dequeueReusableCell(withReuseIdentifier: Constants.CellIDs.SearchKeyword,
                                           for: indexPath) as? SearchKeywordCollectionViewCell else {
      fatalError("Cannot dequeue SearchKeywordCollectionVeiwCell")
    }
