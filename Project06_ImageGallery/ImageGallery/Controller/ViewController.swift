...
class ViewController: UIViewController {
  ...
  var imageList = ["restaurant_detail1", "restaurant_detail2", "restaurant_detail3",
                   "restaurant_detail4", "restaurant_detail5"]
...
extension ViewController : UITableViewDelegate, UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    guard let cell = tableView
      ...
      fatalError("Cannot dequeue CustomTableViewCell")
    }
...
extension ViewController : UICollectionViewDelegate, UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    guard let cell = collectionView
                      .dequeueReusableCell(withReuseIdentifier: collectionViewCellId,
                                           for: indexPath) as? PlaceImageCollectionViewCell else {
      fatalError("Cannot dequeue PlaceImageCollectionViewCell")
    }
    cell.placeImageView.image = UIImage(named:imageList[indexPath.row])
    return cell
  }
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    let imageDetailPageVC = ImageDetailPageViewController(transitionStyle: .scroll,
...
extension ViewController : UIScrollViewDelegate {
  func scrollViewDidScroll(_ scrollView: UIScrollView) {
    if !(scrollView is UICollectionView) {
      ...
      modifiedProfileButtonFrame.origin.x = min(max(initProfileButtonX!,
