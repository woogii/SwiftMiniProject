...
// MARK : - ImageDetailPageViewController: UIPageViewController, UIPageViewControllerDataSource

class ImageDetailPageViewController: UIPageViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate {
  ...
  private func slideToPage(index: Int, viewControllersArray:[DetailImageViewController]) {
    ...
    if currentPageIndex <= index {
      ...
      for i in tempIndex...index {
        ...
        self.setViewControllers([viewControllersArray[i]], direction: .forward, animated: true, completion: {[weak self] (complete: Bool) -> Void in
          if (complete) {
  ...
  func configurePageControlLayout() {
    ...
    view.addConstraint(NSLayoutConstraint(item: pageControl, attribute: .centerX, relatedBy: .equal, toItem: self.view, attribute: .centerX, multiplier: 1, constant: 0))
    
  }

  
  func addBarButtonItem() {
    navigationItem.leftBarButtonItem = UIBarButtonItem(image: UIImage(named:"") , style: .plain, target: self, action: #selector(ImageDetailPageViewController.pushBackButton))
  ...
  func pageViewController(_ pageViewController: UIPageViewController, viewControllerBefore viewController: UIViewController) -> UIViewController? {
    ...
    let currentImageUrl = (viewController as! DetailImageViewController).imageUrl
...
class DetailImageViewController : UIViewController {
  ...
  var imageUrl:String? {
    didSet {
      imageView.sd_setImage(with:URL(string:imageUrl!), placeholderImage: UIImage(named:imageUrl!), options: SDWebImageOptions(), completed: { (image, error, cacheType, url) in
  ...
  func addViewConstraints() {
    ...
    view.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:|-80@750-[v0]-80@1000-|", options: NSLayoutFormatOptions(), metrics: nil, views: ["v0":imageView]))
  }
