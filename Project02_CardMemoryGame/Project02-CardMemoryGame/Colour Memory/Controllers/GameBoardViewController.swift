...
class GameBoardViewController: UIViewController {
  ...
  func fetchAllScores() -> [ScoreList] {
    ...
    do {
      ...
      guard let list = try sharedContext.fetch(fetchRequest) as? [ScoreList] else {
        return [ScoreList]()
      }
      return list
    ...
    for cardButton in cardButtons {
      ...
      if card.isMatched {
        ...
        let time = DispatchTime.now() +
          Double(Int64(Constants.DelayIfMathced * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
  ...
  func placeFippedCardsFaceDown() {
    let time = DispatchTime.now() +
      Double(Int64(Constants.DelayIfNotMathced * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
    ...
    DispatchQueue.main.asyncAfter(deadline: time) {
      ...
      if self.buttonIndices.count >= 2 {
        ...
        for i in 0..<self.selectedCards.count {
          ...
          self.cardButtons[self.buttonIndices[i]].setBackgroundImage(
            UIImage(named:imageName), for: UIControlState())
        }
      }
      // Initialize both arrays
  ...
  func showAlert() {
    ...
    let cancel = UIAlertAction(title: Constants.ActionCancel,
                               style: UIAlertActionStyle.cancel, handler: { (_) -> Void in
                                self.resetGame()
    })
