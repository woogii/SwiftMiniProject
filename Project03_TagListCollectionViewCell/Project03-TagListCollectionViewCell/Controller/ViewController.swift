...
class ViewController: UIViewController {
  ...
  private func configureCollectionViewCellNib() {
    ...
    guard let cell = (cellNib.instantiate(withOwner: nil, options: nil) as NSArray)
                               .firstObject as? TagCollectionViewCell else {
      return
    }
    sizingCell = cell
  }
  ...
  fileprivate func validateEnteredText(_ enteredText: String) -> Bool {
    if enteredText.characters.count > Constants.MaximumKeywordCount {
      let alertController = UIAlertController(title: "",
                                              message: Constants.KeywordValidationErrorMessage,
...
extension ViewController : UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView,
    ...
    guard let cell = tagListCollectionView
                .dequeueReusableCell(withReuseIdentifier: Constants.TagCollectionViewCellIdentifier,
                                     for: indexPath) as? TagCollectionViewCell else {
      fatalError("Unexpected cell configuration")
    }
    configureCell(cell: cell, forIndexPath: indexPath)
    return cell
  }
  ...
  fileprivate func configureCell(cell: TagCollectionViewCell, forIndexPath indexPath: IndexPath) {
    ...
    cell.closeButton.addTarget(self,
                               action: #selector(ViewController.pushTagButton(sender:)),
                               for: .touchUpInside)
  }
  ...
  fileprivate func adjustHeightConstraint(of collectionView: UICollectionView) {
    ...
    UIView.animate(withDuration: 1.0, animations: {
      self.tagCollectionViewHeightConstant.constant =
