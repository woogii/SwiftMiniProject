...
class BookItemTableViewCell: UITableViewCell {
  ...
  private func fetchBookCoverImageUrl(isbn: String) {
    ...
    sharedSession.dataTask(with: bookURL) { (data, _, error) in
      ...
      do {
        guard let jsonDictArray = try JSONSerialization.jsonObject(with: data,
        ...
        guard let volumeInfoDict =
          dictArray[0][Constants.GoogleBookAPI.JSONResponseKeys.VolumeInfo] as? [String:Any] else {
            return
        }
        guard let coverImageInfoDict =
          volumeInfoDict[Constants.GoogleBookAPI.JSONResponseKeys.ImageLinks] as? [String:Any] else {
            return
        }
        guard let smallThumbnailUrl =
          coverImageInfoDict[Constants.GoogleBookAPI.JSONResponseKeys.SmallThumbnail] as? String
          else {
          return
        }
        self.bookCoverImageView.sd_setImage(with:URL(string:smallThumbnailUrl)!,
