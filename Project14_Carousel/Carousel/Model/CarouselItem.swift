...
struct CarouselItem {
  ...
  init?(dictionary: [String:Any]) {
    ...
    guard let imageName = dictionary[Constants.JSONKeys.Image] as? String,
            let title = dictionary[Constants.JSONKeys.Title] as? String else {
      return nil
    }
  ...
  static func getCarouselItemListFromBundle() -> [CarouselItem]? {
    ...
    guard let url = Bundle.main.url(forResource: Constants.ResourceName,
                                    withExtension: Constants.ResourceExtension) else {
      return nil
    }
